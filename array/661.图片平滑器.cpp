    ...
    vector<vector<int>> imageSmoother(vector<vector<int>>& img) {
        ...
        for (int i = 0; i < m; ++i) {
            for (int j = 0; j < n; ++j) {
                if (m == 1) {
                    if (j == 0)
                        res[i][j] = (img[i][j] + img[i][j+1]) / 2; 
                    else if (j == n-1)
                        res[i][j] = (img[i][j] + img[i][j-1]) / 2;
                    else
                        res[i][j] = (img[i][j-1] + img[i][j] + img[i][j+1]) / 3;
                } else if (n == 1) {
                    if (i == 0)
                        res[i][j] = (img[i][j] + img[i+1][j]) / 2; 
                    else if (i == m-1)
                        res[i][j] = (img[i][j] + img[i-1][j]) / 2;
                    else
                        res[i][j] = (img[i-1][j] + img[i][j] + img[i+1][j]) / 3;
                } else {
                    if (i == 0 && j == 0)
                        ...
                        res[i][j] = res[i][j] = (img[i-1][j-1] + img[i-1][j] + img[i-1][j+1] + img[i][j-1] + img[i][j] + img[i][j+1] + img[i+1][j-1] + img[i+1][j] + img[i+1][j+1]) / 9;
                }
            }
        }
        return res;
